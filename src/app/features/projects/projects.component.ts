...
export class ProjectsComponent implements OnInit {
    ...
    this.projectsService.createProject(payload).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (newProj) => {
        ...
        if (requirements.length > 0) {
          ...
          forkJoin(reqRequests).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
  ...
  onDeleteProject(projectId: string, event: Event) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.loading.set(true);
        this.projectsService.deleteProject(projectId).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
          next: () => {
  ...
  onToggleStatus(project: Project, event: Event) {
    ...
    this.projectsService.updateProject(project.id, { isActive: newStatus }).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: (updatedProj) => {
  ...
  onRegisterDevolution() {
    ...
    this.isSubmitting.set(true);
    this.projectsService.registerDevolution(project.id, notes).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
      next: () => {
